...
class Piano:
    ...
    def __prepare_piano (self, root, octaves, lowest_octave):
        ...
        piano_frame = ctk.CTkFrame(master=root, width=frame_width, height=frame_height, bg_color="white")
        ...
        def create_white_keys():
            ...
            for i in range (octaves*white_num):
                ...
                new_key = Key(master=piano_frame, note=white_notes[i], parent_piano=self, bg="white", text="", borderwidth=2)
                new_key.configure(command=new_key.clicked)
                new_key.grid(row=0, column=i*3, rowspan=2, columnspan=3, sticky='nsew')
        ...
        def create_black_keys():
            ...
            for i in range (octaves*white_num-1):
                if black[i]:
                    ...
                    new_key = Key(master=piano_frame, note=black_notes[name_ix], parent_piano=self, bg="black", text="", borderwidth=2)
                    new_key.configure(command=new_key.clicked)
                    new_key.grid(row=0, column=(i*3)+2, rowspan=1, columnspan=2, sticky='nsew')
