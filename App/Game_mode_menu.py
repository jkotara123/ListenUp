...
class GameModeMenu:
    ...
    def __prepare_quiz_manager (self, game_mode_prompt):
        if game_mode_prompt is not None:
            if game_mode_prompt == "Interval":
                ...
                self.quiz_manager_comm_channel.create_quiz_manager("Interval", self.piano, max_listens)
    ...
    def play_question (self, unused):
        ...
        def internal_play_question ():
            ...
            if self.listens_left == 0:
                self.listens_left_label.configure(text=f"You cannot listen listen to the question anymore")
            elif self.listens_left == 1:
                self.listens_left_label.configure(text=f"You can listen to the question {self.listens_left} more time")
            else:
                self.listens_left_label.configure(text=f"You can listen to the question {self.listens_left} more times")

            for i in range (1, listening_steps+1):
                play_image = Image.open(f"{other_resources_path}/play_sound/play_sound{i}.png")
            ...
            if self.listens_left == 0:
                play_image = Image.open(f"{other_resources_path}/play_sound/play_sound_grey.png")
                play_image = play_image.resize((play_label_width, play_label_height))
                play_image = ImageTk.PhotoImage(play_image)
                self.play_label.configure(image=play_image)
                # self.menu_frame.winfo_toplevel().update()
            else:
                play_image = Image.open(f"{other_resources_path}/play_sound/play_sound{0}.png")
                play_image = play_image.resize((play_label_width, play_label_height))
                play_image = ImageTk.PhotoImage(play_image)
                self.play_label.configure(image=play_image)
                # self.menu_frame.winfo_toplevel().update()
    ...
    def answered_correctly (self):
        ...
        self.next_question_button.configure(text_color="black", hover_color="grey", border_color="black")
        self.show_answer_button.configure(text_color="black", hover_color="grey", border_color="black")
    ...
    def next_question (self):
        if self.answer_given:
            ...
            self.question_counter += 1
            self.listens_left = max_listens
            self.next_question_button.configure(text_color="grey", hover_color="white", border_color="grey")
            self.show_answer_button.configure(text_color="grey", hover_color="white", border_color="grey")
            self.listens_left_label.configure(text=f"You can listen the question {self.listens_left} more times")
    ...
    def __prepare_menu (self, root, octaves):
        ...
        menu_frame = ctk.CTkLabel(master=root, width=width, height=height, bg_color="white", fg_color="white", text="white")
        menu_frame.pack(fill=ctk.BOTH)

        menu_bg_image = Image.open(f"{other_resources_path}/gm_background_.png")
        ...
        menu_bg_image = Image.open(f"{other_resources_path}/gm_background_rev_.png")
        menu_bg_image = ImageTk.PhotoImage(menu_bg_image)
        bg_image_label = tk.Label(menu_frame, image=menu_bg_image, text="", bg="white")
        bg_image_label.place(x=width-menu_bg_image.width(), y=0, relheight=1)
        bg_image_label.image = menu_bg_image

        correct_ans_image = ctk.CTkImage(light_image=Image.open(f"{other_resources_path}/white_tick_tbg.png"), size=(button_height//2, button_height//2))
        ...
        incorrect_ans_image = ctk.CTkImage(light_image=Image.open(f"{other_resources_path}/white_cross_tbg.png"), size=(button_height//2, button_height//2))
        ...
        play_image = ctk.CTkImage(light_image=Image.open(f"{other_resources_path}/play_sound/play_sound0.png"), size=(play_label_width, play_label_height))
        play_label = ctk.CTkLabel(master=menu_frame, text="", image=play_image, width=play_label_width, height=play_label_height)
        ...
        show_answer_button = ctk.CTkButton(master=menu_frame, text=f"Show the correct answer", font=(fontname, 12), fg_color='white', bg_color="transparent",
                                           ...
                                           width=button_width//2, height=button_height//2, border_color="grey", border_width=2)
        show_answer_button.place(relx=0.5, y=0.85*(vertical_spacing+height), anchor=ctk.CENTER)
