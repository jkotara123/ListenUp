class QuizManager:
    ...
    def create_new_question (self):
        ...
        if self.question_handler.one_left():
            self.piano_manager.set_key_color(self.question_handler.get_expected(), "gold")
        else:
            self.piano_manager.set_key_color(self.question_handler.get_expected(), "green")
    ...
    def show_correct_answer (self):
        ...
        for i in range (0, len(sequence)):
            if k < len(to_show) and to_show[k] == i:
                ...
                add_greens.append(i)
        self.piano_manager.play_sequence(sequence, to_show, expected=None, one_left=one_left, time_gaps=time_gaps, add_greens=add_greens)
        # self.listens += 1
        self.piano_manager.enable()
    ...
    def play_question (self):
        if self.listens < self.max_listens:
            ...
            one_left = self.question_handler.one_left()
            time_gaps = self.question_handler.get_time_gaps()
            self.piano_manager.play_sequence(sequence, to_show, expected, one_left, time_gaps)
            self.listens += 1
            self.piano_manager.enable()
        else:
    ...
    def check_answer (self, note_name):
        ...
        self.piano_manager.set_key_color(self.question_handler.get_previously_expected(), "red")
