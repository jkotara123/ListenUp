...
class ForgotPasswordMenu:
    ...
    def __send_reset_code_mail (self):
        ...
        reset_code_length = 6
        reset_code = "".join(random.choices(reset_code_characters, k=reset_code_length))
        ...
        if self.database_manager.see_if_username_exists(username):
            ...
            message = (f"Hello!\n"
                       f"It seems that you requested that an account tied to this email address ({username} to be specific) "
                       f"has its password changed\n"
                       f"Your reset password code is as follows: {reset_code}\n\n"
                       f"This message was generated and sent automatically, do not reply to it\n")
            self.mail_sender.send_mail(rec=mail, subject="Password reset", message=message)
    ...
    def __reset_password (self):
        ...
        if new_password != repeat_password:
            self.post_rse_error_label.configure(text="Repeated password and password are not the same")
        else:
            self.post_rse_error_label.configure(text="")
            self.database_manager.reset_users_password(self.login_entry.get(), new_password)
    ...
    def __prepare_menu (self, root):
        ...
        pre_reset_menu_frame = ctk.CTkFrame(master=root, width=width, height=height)
        ...
        send_reset_code_button = ctk.CTkButton(master=pre_reset_menu_frame, text_color="black", font=(fontname, 14),
                                               ...
                                               width=100, height=10)
        send_reset_code_button.place(relx=0.5, rely=top+3*padding, anchor=ctk.CENTER)
        ...
        reset_code_button.place(relx=0.5, rely=top+6*padding, anchor=ctk.CENTER)
        ...
        post_reset_menu_frame = ctk.CTkFrame(master=root, width=width, height=height)
        ...
        repeat_password_label.place(relx=0.5, rely=top+3*padding, anchor=ctk.CENTER)

        repeat_password_entry = ctk.CTkEntry(master=post_reset_menu_frame, text_color="black", font=(fontname, 14),
                                        fg_color="white", bg_color="white", corner_radius=8, border_color="black",
                                        border_width=2, show='*')
        repeat_password_entry.place(relx=0.5, rely=top+4*padding, anchor=ctk.CENTER)
        ...
        view_password_checkbox = ctk.CTkCheckBox(master=post_reset_menu_frame, text_color="black", checkbox_width=22,
                                                 ...
                                                 corner_radius=32)
        view_password_checkbox.place(relx=0.5, rely=top+5*padding, anchor=ctk.CENTER)
        ...
        reset_password_button.place(relx=0.5, rely=top+6*padding, anchor=ctk.CENTER)
