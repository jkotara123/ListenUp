...
class SignUpMenu:
    def __init__(
        ...
        self.new_user_username: str = None
        self.new_user_mail: str = None
        self.new_user_password: str = None
        self.mail_sender: MailSender = self.__create_mail_sender()
        self.verification_code: str = None
        self.pre_mail_send_menu_frame: ctk.CTkFrame = None
        self.post_mail_send_menu_frame: ctk.CTkFrame = None
        self.login_entry: ctk.CTkEntry = None
        self.mail_entry: ctk.CTkEntry = None
        self.password_entry: ctk.CTkEntry = None
        self.repeat_password_entry: ctk.CTkEntry = None
        self.activation_code_entry: ctk.CTkEntry = None
        self.pre_ms_error_label: ctk.CTkLabel = None
        self.post_ms_error_label: ctk.CTkLabel = None
        self.view_password_checkbox: ctk.CTkCheckBox = None
        self.__prepare_menu(root)
        if launch_immediately:
            root.mainloop()

    def __sign_in_new_user(self) -> None:
        ...
        if password != repeated_password:
            self.pre_ms_error_label.configure(text="Passwords do not match")
        else:
        ...
        if not username_unique:
            self.pre_ms_error_label.configure(
                text="Username is already in use by someone else"
            )
        else:
            self.pre_ms_error_label.configure(text="")
    ...
    def send_mail_with_verification_code(self) -> None:
        activation_code_characters = string.ascii_letters + string.digits
        activation_code_length = 6
        activation_code = "".join(
            random.choices(activation_code_characters, k=activation_code_length)
        )
        ...
        self.mail_sender.send_mail(
            rec=self.new_user_mail, subject="Signing up for ListenUp", message=message
        )
    ...
    def __quit_code_verification_stage(self) -> None:
    ...
    def __finalize_registration(self) -> None:
    ...
    def __prepare_menu(self, root: ctk.CTk) -> None:
        width = 400
        height = 500
        ...
        menu_bg_image = ctk.CTkImage(
            light_image=Image.open(f"{images_path}/sim_bg.png"), size=(width, height)
        )
        image_label = ctk.CTkLabel(
            master=pre_mail_send_menu_frame, image=menu_bg_image, text=""
        )
        ...
        upper_label = ctk.CTkLabel(
            master=pre_mail_send_menu_frame,
            text_color="black",
            font=(fontname, 34, "bold"),
            fg_color="white",
            bg_color="white",
            text="Please sign up",
        )
        ...
        view_password_checkbox.place(
            relx=0.5, rely=top + 9.5 * padding, anchor=ctk.CENTER
        )
        ...
        post_mail_send_menu_frame = ctk.CTkFrame(
        ...
        image_label = ctk.CTkLabel(
            master=post_mail_send_menu_frame, image=menu_bg_image, text=""
        )
        image_label.pack(fill=ctk.BOTH, expand=True)
        ...
        upper_label = ctk.CTkLabel(
            master=post_mail_send_menu_frame,
            text_color="black",
            font=(fontname, 34, "bold"),
        ...
        activation_code_label = ctk.CTkLabel(
            master=post_mail_send_menu_frame,
            text_color="black",
            font=(fontname, 14),
            text="Activation code:",
        ...
        finalize_registration_button = ctk.CTkButton(
            master=post_mail_send_menu_frame,
            text_color="black",
        ...
        finalize_registration_button.place(
        ...
        go_back_button = ctk.CTkButton(
            ...
            hover_color="grey",
            height=10,
            command=self.__quit_code_verification_stage,
        )
        go_back_button.place(relx=0.15, rely=0.05, anchor=ctk.CENTER)
