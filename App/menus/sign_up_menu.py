...
class SignUpMenu:
    ...
    def __sign_in_new_user(self):
        ...
        if password != repeated_password:
            self.pre_ms_error_label.configure(
                text="Repeated password must be the same as password")
        else:
            self.pre_ms_error_label.configure(text="")
        username_unique = not self.database_manager.see_if_username_exists(
            username)
    ...
    def send_mail_with_verification_code(self):
        ...
        activation_code = "".join(random.choices(
            activation_code_characters, k=activation_code_length))
    ...
    def __finalize_registration(self):
        ...
        if self.verification_code != self.activation_code_entry.get():
            self.post_ms_error_label.configure(
                text="Incorrect activation code")
        else:
    ...
    def __prepare_menu(self, root):
        ...
        pre_mail_send_menu_frame = ctk.CTkFrame(
        ...
        menu_bg_image = ctk.CTkImage(light_image=Image.open(f"{images_path}/sim_bg.png"),
                                     size=(width, height))
        ...
        repeat_password_entry.place(
            relx=0.5, rely=top + 8*padding, anchor=ctk.CENTER)
        ...
        sign_in_button.place(relx=0.5, rely=top + 11 *
        ...
        activation_code_label.place(
            relx=0.5, rely=top+padding, anchor=ctk.CENTER)
        ...
        activation_code_entry.place(
            relx=0.5, rely=top+2*padding, anchor=ctk.CENTER)
