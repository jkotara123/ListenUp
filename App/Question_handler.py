...
class QuestionHandler:
    ...
    def create_new_question (self):
        self.question = modes[self.prompt](self.octaves, self.lowest_octave, create_random=True)
        print(self.question.get_sequence())
        self.index = 1


    def get_expected (self):
        return self.question.get_sequence()[self.index]


    def get_previously_expected (self):
        return self.question.get_sequence()[self.index-1]
    ...
    def check_answer (self, note_name):
        if self.question.get(self.index) == note_name:
            self.index += 1
