...
class Statistics_menu:
    def __init__(self, root, user: str, database_manager: DatabaseManager, launch_immediately=True):
    ...
    def __prepare_menu(self, root):
        ...
        for i, mode in enumerate(["Interval", "Chord"]):
            ...
            correct_label = ctk.CTkLabel(
                master=menu_frame, text="Correct: " + str(correct_ans), font=(fontname, 20))
            correct_label.place(relx=0.3, rely=0.33+i*0.35, anchor='center')

            incorrect_label = ctk.CTkLabel(
                master=menu_frame, text="Incorrect: " + str(incorrect_ans), font=(fontname, 20))
            incorrect_label.place(relx=0.3, rely=0.43+i*0.35, anchor='center')
            percentage_label = ctk.CTkLabel(
                master=menu_frame, text=f"Correct %:\n {correct_percentage:.2f}%", font=(fontname, 20))
        ...
        go_back_button = ctk.CTkButton(master=menu_frame, width=width*0.1, height=10, border_width=2, border_color="black",
                                       fg_color="white", text_color="black", text="Go back", font=(fontname, 14),
                                       hover_color="grey",
